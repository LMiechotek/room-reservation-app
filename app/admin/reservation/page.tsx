    ...
    try {
      ...
      if (editingId) {
        ...
        for (const aulaNumero of sorted) {
          ...
          if (!response.ok)
            throw new Error(
              result?.message || `Erro ao criar aula ${aulaNumero}`
            );
  ...
  const handleCancelReservation = async () => {
    ...
    try {
      const response = await fetch(
        `/api/reservations/${selectedReservation.id}/cancel`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
          body: JSON.stringify({
            cancelado_por: selectedReservation.usuario_id,
          }),
        }
      );
  ...
  return (
    <>
      <ConfirmModal
        ...
        description={`Deseja realmente cancelar a reserva da disciplina de ${selectedReservation?.disciplina || "esta reserva"
          }?`}
        confirmText={deleting ? "Cancelando..." : "Sim, cancelar"}
        cancelText="Não"
        onCancel={() => {
          setShowConfirmModal(false);
          setSelectedReservation(null);
